...
def construct_local_frame(positions, box, axis_types, axis_indices, lmax=2):
    '''
    ...
    Inputs:
        ...
        axis_types, axis_indices: 
            local coordinate system construction rules
        lmax:
    ...
    # Z-Only
    Zonly_indices = (axis_types == Zonly)
    if np.sum(Zonly_indices) > 0:
        size_filter = (jnp.abs(vec_z[:,0]) < jnp.sqrt(0.75))
        filter1 = jnp.logical_and(Zonly_indices, size_filter)
        filter2 = jnp.logical_and(Zonly_indices, jnp.logical_not(size_filter))
        vec_x = vec_x.at[filter1].set(np.array([1., 0., 0.]))
        vec_x = vec_x.at[filter2].set(np.array([0., 1., 0.]))
    # for those that are not Z-Only, get normalized vecX
    not_Zonly_indices = jnp.logical_not(Zonly_indices)
    vec_x_not_Zonly = positions[x_atoms[not_Zonly_indices]] - positions[not_Zonly_indices]
    vec_x_not_Zonly = pbc_shift(vec_x_not_Zonly, box, box_inv)

    vec_x = vec_x.at[not_Zonly_indices].set(normalize(vec_x_not_Zonly, axis=1))
    # Bisector
    Bisector_indices = (axis_types == Bisector)
    if np.sum(Bisector_indices) > 0:
        vec_z_Bisector = vec_z[Bisector_indices] + vec_x[Bisector_indices]
        vec_z = vec_z.at[Bisector_indices].set(normalize(vec_z_Bisector, axis=1))
    # z-bisector
    ZBisect_indices = (axis_types == ZBisect)
    if np.sum(ZBisect_indices) > 0:
        vec_y_ZBisect = positions[y_atoms[ZBisect_indices]] - positions[ZBisect_indices]
        vec_y_ZBisect = pbc_shift(vec_y_ZBisect, box, box_inv)
        vec_y_ZBisect = normalize(vec_y_ZBisect, axis=1)
        vec_x_ZBisect = vec_x[ZBisect_indices] + vec_y_ZBisect
        vec_x = vec_x.at[ZBisect_indices].set(normalize(vec_x_ZBisect, axis=1))
    # ThreeFold
    ThreeFold_indices = (axis_types == ThreeFold)
    if np.sum(ThreeFold_indices) > 0:
        vec_x_threeFold = vec_x[ThreeFold_indices]
        vec_z_threeFold = vec_z[ThreeFold_indices]
        
        vec_y_threeFold = positions[y_atoms[ThreeFold_indices]] - positions[ThreeFold_indices]
        ...
        vec_y = vec_y.at[ThreeFold_indices].set(vec_y_threeFold)
        vec_z = vec_z.at[ThreeFold_indices].set(vec_z_threeFold)
...
pme_reciprocal_force = jax.jit(jax.grad(pme_reciprocal, argnums=0), static_argnums=(4,5,6,7))


def pme_reciprocal_energy_from_lh(positions, box,  Q_lh, kappa, lmax, K1, K2, K3, axis_types, axis_indices):
    localframes = construct_local_frame(positions, box, axis_types, axis_indices, lmax)
    Q_gh = rot_local2global(Q_lh, localframes, lmax)
    return pme_reciprocal(positions, box, Q_gh, kappa, lmax, K1, K2, K3)

pme_reciprocal_force_alt = jax.grad(pme_reciprocal_energy_from_lh, argnums=0)